...
- **Boot/shutdown resilience**: `etc/systemd/system.conf.d/10-bounded-shutdown.conf` (`DefaultTimeoutStopSec=15s`) plus per-service `10-bounded-stop.conf` drop-ins for `dbus`, `polkit` and `NetworkManager` — stock Debian lets systemd *extend* a hung service's stop timeout (5min → 14min observed), turning one reboot into a 10+ minute apparent hang. `10-watchdog.conf` arms the gs201 hardware watchdog (`RuntimeWatchdogSec=15s`) so a post-boot hang self-resets. `NetworkManager.service.d/10-boot-resilience.conf` caps NM's 10-minute default start timeout at 90s and adds `Restart=on-failure`, so a slow/flapping/absent USB-Ethernet dongle can't hold the boot "in progress" (which floods the UART console and drowns the login prompt).
- **Network self-heal**: `netcheck-recover.service` + `usr/local/sbin/netcheck-recover` — waits 90s for a global-scope DHCP address; on success marks the slot successful, on failure rolls back to a known-good boot slot (or plain-reboots). Guarded two ways: it exits without rebooting when no non-loopback NIC exists at all (bench/UART-only boot, dongle unplugged), and a persistent counter under `/var/lib/netcheck-recover` caps it at 3 attempts so a dead dongle parks the device in a debuggable state instead of reboot-looping.
...
- **USB gadget**: `usb-gadget.service` + `99-usb-gadget.rules` + `usr/local/sbin/usb_gadget` bring up a CDC-NCM(+ACM) gadget so the device is reachable over USB-Ethernet when the wired dongle isn't there. The unit is gated on `ConditionPathExistsGlob=/sys/class/udc/*`, so it no-ops on a dongle (host-mode) boot and can't disturb the ethernet+charging path; the udev rule re-triggers it on UDC hot-plug, because the cable gets moved between dongle and PC at runtime. The script discovers the UDC from sysfs rather than hardcoding `11210000.dwc3`, and is idempotent (descriptor attrs return `-EIO` once functions are linked, so a re-run only re-binds).

  **`usb0` is addressed by the script, not NetworkManager**, and that is deliberate. For CDC-NCM the host only sees carrier once the device's `usb0` is administratively up, but NM won't autoconnect a carrier-less device — circular, and the measured result was `carrier=0` on the host for a full 60s sample with the device never even attempting DHCP. So `usb_gadget` does `ip link set usb0 up` + `ip addr replace 10.42.0.1/24` itself, and `etc/NetworkManager/conf.d/10-unmanage-usb0.conf` marks `usb0` unmanaged so the **wildcard** `default_connection` profile (no `ifname`, matches any ethernet) can't claim it, run DHCP on a cable with no DHCP server, and flush the address on timeout. Host side: put `10.42.0.2/24` on the peer interface and `ssh` to `10.42.0.1`.
