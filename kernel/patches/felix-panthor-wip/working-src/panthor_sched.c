...
queue_run_job(struct drm_sched_job *sched_job)
{
	...
	u64 call_instrs[NUM_INSTRS_PER_SLOT] = {
		...
		/* WAIT(all) => wait for the user CS (compute) to complete before
		 * we flush its output.
		 */
		(3ull << 56) | (waitall_mask << 16),

		/* FELIX NON-COHERENT FIX: the shader's output sits in GPU L2 and
		 * is never written back to DRAM on a non-coherent system, so the
		 * CPU reads stale data after clFinish. Clean (+invalidate) the GPU
		 * caches to DRAM *before* signalling the completion fence.
		 *
		 * MOV32 rX+2, cs.latest_flush (reload; CALL clobbered the regs)
		 */
		(2ull << 56) | (val_reg << 48) | job->call_info.latest_flush,

		/* FLUSH_CACHE2.clean_inv_all.no_wait.signal(0) rX+2 [output WB] */
		(36ull << 56) | (0ull << 48) | (val_reg << 40) | (0 << 16) | 0x233,

		/* WAIT(0) => wait for the output FLUSH_CACHE2 to complete */
		(3ull << 56) | (1 << 16),
