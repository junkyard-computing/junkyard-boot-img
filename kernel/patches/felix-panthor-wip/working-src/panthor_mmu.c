...
static int panthor_gpuva_sm_step_map(struct drm_gpuva_op *op, void *priv)
{
	...
	prot = flags_to_prot(vma->flags);

	/*
	 * FELIX NON-COHERENT FIX: on a non-coherent system the GPU's L2 is not
	 * snooped by (and does not snoop) the CPU. userspace fills shader
	 * descriptors / kernel args through a write-combine CPU mapping that
	 * goes straight to DRAM, but if the GPU caches those reads in its L2 it
	 * can return cache-line-granular stale data (observed: the buffer
	 * descriptor reads back as a NULL descriptor while the adjacent sampler
	 * descriptor in the same page is correct), so the compute shader writes
	 * to a null pointer / produces no output. Map such buffers uncached on
	 * the GPU so every access hits DRAM, which the WC writes keep coherent.
	 *
	 * Keep *executable* (shader code) mappings cached: they are written once
	 * at program-build time (not rewritten per-submit, so not subject to the
	 * descriptor race) and the per-job start FLUSH_CACHE2 invalidate makes
	 * any stale GPU-cache line moot. Uncaching shader code would force every
