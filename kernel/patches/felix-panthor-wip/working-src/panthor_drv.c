...
{
	/*
	 * FELIX NON-COHERENT FIX: userspace fills the command stream, shader
	 * descriptors and kernel args through a write-combine mapping, then
	 * issues this SUBMIT ioctl. The GPU, however, consumes that memory
	 * later from the drm_sched worker thread (a different CPU), whose
	 * wmb()+doorbell in queue_run_job only drains *its* write buffer, not
	 * the submitting CPU's. Drain this (submitting) CPU's write-combine
	 * buffer here so the WC writes have reached DRAM before the job becomes
	 * visible to the scheduler/GPU. Without it the GPU intermittently reads
	 * stale (zero) descriptors and produces no compute output.
	 */
	wmb();
